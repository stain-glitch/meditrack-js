...
router.post("/", auth, async (req, res) => {
  try {
    ...
    const { name, role, facility, password, vehicle } = req.body;
    if (!name || !role) return res.status(400).json({ error: "name and role required" });
    if (!VALID_ROLES.includes(role))
      return res.status(400).json({ error: `role must be one of: ${VALID_ROLES.join(", ")}` });
    if (role === "Transporter" && !vehicle?.numberPlate)
      return res.status(400).json({ error: "Transporters require a vehicle number plate" });
    ...
    // 6. Save vehicle details if Transporter
    if (role === "Transporter" && vehicle?.numberPlate) {
      await pool.query(
        `INSERT INTO transporter_vehicles (wallet, number_plate, model, manufacturer, contract_type)
         VALUES ($1,$2,$3,$4,$5)
         ON CONFLICT (wallet) DO UPDATE SET
           number_plate=$2, model=$3, manufacturer=$4, contract_type=$5`,
        [wallet.toLowerCase(), vehicle.numberPlate, vehicle.model || "", vehicle.manufacturer || "", vehicle.contractType || ""]
      );
    }
...
// GET /api/users/transporters — returns transporters with vehicle details
router.get("/transporters/vehicles", auth, async (req, res) => {
  try {
    const r = await pool.query(`
      SELECT cu.wallet, cu.name, cu.facility,
             tv.number_plate, tv.model, tv.manufacturer, tv.contract_type
      FROM chain_users cu
      LEFT JOIN transporter_vehicles tv ON LOWER(tv.wallet)=LOWER(cu.wallet)
      WHERE cu.role='Transporter' AND cu.active=true
      ORDER BY cu.name ASC
    `);
    res.json({ transporters: r.rows });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// GET /api/users/receivers — returns users who can receive batches
router.get("/receivers/list", auth, async (req, res) => {
  try {
    const r = await pool.query(
      `SELECT wallet, name, role, facility FROM chain_users
       WHERE role IN ('Pharmacist','HSA','CMST') AND active=true
       ORDER BY name ASC`
    );
    res.json({ receivers: r.rows });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
